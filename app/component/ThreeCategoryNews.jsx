...
export default function ThreeCategoryNews() {
  ...
  const publishedNews = data.articles.filter((article)=> article.published)
  const investigationNews = publishedNews.filter((article) => article.category === "Investigation")
  const politicsNews = publishedNews.filter((article) => article.category === "Politics")
  const sportsNews = publishedNews.filter((article) => article.category === "Sports")
        ...
        <div className="border border-gray-300 px-5 pt-5">
          ...
          {sportsNews.slice(0, 4).map((article) => (
            ...
            >
              ...
              {/* Image */}
              {article.image && (
                <div className="relative group overflow-hidden w-24 h-24">
                  <Image
                    src={article.image}
                    alt={article.title}
                    fill
      ...
      <div className="lg:col-span-3 sticky top-20 self-start mr-5 p-6 border border-gray-300">
        ...
        <div className="space-y-4">
          {investigationNews.slice(0, 4).map((article) => (
            <div key={article.slug} className="pb-2 border-b border-gray-300 last:border-b-0">
